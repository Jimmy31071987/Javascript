...
namespace WebApp.WebControls
{
    public partial class ctrl_tweets_N_links : System.Web.UI.UserControl
    {
        public string query = "@username";
        ...
        public void findUserTwitter(string resource_url, string q)
        {
            ...
            try
            {
                ...
                twitter.InnerHtml = objText;/**/
               
                html = "<ul>";
                JArray jsonDat = JArray.Parse(objText);
                for (int x = 0; x < 4; x++)
                {
                    if (jsonDat[x]["text"] != null && jsonDat[x]["text"].ToString().Trim() != "")
                    {
                        //html += jsonDat[x]["id"].ToString() + "<br/>";
                        html += "<li>" + jsonDat[x]["text"].ToString() + "<li/>";
                        // html += jsonDat[x]["name"].ToString() + "<br/>";
                    }

                }

                twitter.InnerHtml = html+"</ul>";
            }
            catch 
            {
                twitter.InnerHtml = html + "Tweets are loading.....";
            }
        }
    }
}
